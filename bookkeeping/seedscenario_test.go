...
func TestSeedScenario_ProjectsEveryEntityViaEvents(t *testing.T) {
	...
	scn := accounting.Scenario{
		...
		Counterparties: []accounting.Counterparty{{ID: "CP-0001", Name: "TSMC", Kind: accounting.CounterpartyCustomer, Active: true}},
	}
	...
	cps, _ := repo.Counterparties(ctx)
	if len(cps) != 1 || cps[0].ID != "CP-0001" {
		t.Fatalf("counterparty not projected: %+v", cps)
	}
}
