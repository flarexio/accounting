...
**沒有選擇畫面**——`ledger tui` 直接進入第一個分公司(seed 順序第一個,`hq` 台北總公司)的 chat。分公司在 session 內用 `/branch` 切換。

| 操作 | 預期行為 |
|------|----------|
| 啟動 | 短暫 `Connecting to ledger...` 後直接進 chat,header 顯示目前分公司名 |
| 輸入 `/help` | 系統行列出可用指令(`/branch`、`/counterparties`、`/help`)|
...
| 輸入 `/cp`（或 `/counterparties`） | 系統行列出目前所有 counterparty(`CP-id`、名稱、kind、active/inactive、tax_id)|
| 輸入 `/cp add` | 開啟**新增表單** overlay:`tab`/`↑`/`↓` 切換欄位、`Enter` 儲存、`Esc` 取消;name 必填;kind 是 `←`/`→` 切換的選項(`customer`/`supplier`/`both`),打不進非法值 |
| 表單填妥 name、選好 kind 後 `Enter` | 配到下一個 `CP-NNNN`(緊接現有編號、不跳號),系統行顯示「registered CP-00xx …」;再 `/cp` 可看到新列,`find_counterparties` 也立即解析得到 |
| 表單 name 留空就 `Enter` | 表單就地顯示錯誤,不新增 |
| `/cp add` 用已存在的 tax_id | 表單顯示錯誤「already registered: tax_id … is CP-00xx」,不新增 |
| 輸入 `/不存在` | 系統行顯示「unknown command — type /help」,不送 agent |
| `ctrl+c` / `ctrl+d` | 程式結束(唯一結束方式)|

> `/branch` 切換等於用新分公司重連——對話會清空(recall 記憶也重置),這是預期的。operator 分公司會帶進 prompt,影響預設 branch_id。
>
> `/cp` 是 operator 建立主檔的入口(不是 agent 工具):agent 只用 `find_counterparties` 解析既有對象,新增一律由人在 `/cp add` 慎重輸入。tax_id 不做格式驗證(非僅台灣場景),只擋重複。
...
| `ESC`（turn 完成後） | 無 spinner 時 | 重連目前分公司的全新 session（清空對話）；沒有選擇畫面可退 |
| `ctrl+c` / `ctrl+d` | 任何時候 | 程式結束（唯一的結束方式）|
...
- [ ] 啟動直接進 chat（無選擇畫面）；`/help` 列指令、`/branch <id>` 能切換分公司並清空對話
- [ ] `/cp` 列出 counterparty；`/cp add` 表單能新增（配到下一個 `CP-NNNN`、不跳號），重複 tax_id 被擋，新增後 `find_counterparties` 立即解析得到
