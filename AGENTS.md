...
- **Period-end closing is rule-driven, not LLM-driven.** `ClosePeriod` is a `bookkeeping` use case but deliberately not a `bookkeeping.Intent` — closing policy is too company-specific to delegate to a model. The trigger is an external scheduler (cron, k8s `CronJob`) invoking `ledger close --period <id>`. The use case refuses to close before `Period.End` has actually passed in `Company.TimeZone`, even when the cron expression itself runs in UTC.
