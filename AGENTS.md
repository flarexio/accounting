...
- **Counterparty lookup is lexical.** `Counterparty` (customer/supplier) is event-sourced master data — projected from `CounterpartyAdded` by `ApplyCounterparty` — and seeds like accounts/branches (`Scenario.Counterparties` / a `counterparties:` block in the seed file). Runtime additions are **operator-driven, not agent-driven**: the ledger TUI's `/counterparties` (alias `/cp`) command lists the master data and its `add` form runs the `AddCounterparty` use case, which allocates the next dense `CP-NNNN` id, registers it active, and rejects a `tax_id` already in use (no `tax_id` format check — not jurisdiction-specific). Like the other reference-data writes its projection is an idempotent upsert-by-id with no per-subject sequence, so there is no optimistic-concurrency check and it expects a single writer; because the projection is upsert-by-id, enriching a counterparty later (e.g. adding a missing `tax_id`) is a re-publish of `CounterpartyAdded` under the existing id — a natural extension, though the current `/cp` form only adds. The agent never fabricates master data — `find_counterparties` only resolves an existing name, alias, or tax id to its `CP-id`. Unlike `find_accounts` it ranks in the agent over `repo.Counterparties()` (`CounterpartyMatch` tiers: exact id/name/tax-id/alias, then substring) rather than a hybrid index — counterparties are directly-named entities of low cardinality, so semantic ranking adds little over alias coverage.
- **Invoices/receipts ride on the journal entry, not a separate aggregate.** A line carries an optional `Dimensions.CounterpartyID` (lines that set it on one entry share one value; empty on cash/tax/internal lines) — this attributes AR/AP and drives aging. An entry carries an optional `SourceDoc` (`Source`: kind invoice/bill/receipt + number) recording the document it came from. The validator checks a referenced counterparty exists and is active, that an entry mixes no two counterparties, and that a set `SourceDoc.Kind` is known. There is **no** `Invoice` aggregate or status machine: "is it paid / what is outstanding" is a derived query over entries + the `settles` relation, not stored state. A payment is recorded with the `settle` intent (`SettleJournal`): it posts the receipt entry and a `settles` `JournalRelation` from the receipt to the invoice's JE-id in one `JournalPosted`, mirroring how `reverse_journal` posts an entry + a `reverses` relation. `RelationsTo(invoiceID)` answers "what settled this invoice".
...
- **Distillation capture is a side-channel, not the event stream.** Opt-in via `llm.dataset_path`: when set, `tui` and `book-run` append each *clean* bookkeeping run (`Book` returned no error — committed or cleanly rejected; aborted/failed runs are dropped, which is rejection-sampling for free) to a JSONL corpus in the `dataset` package, one `Record` per line. `book-run` makes capture scriptable for batch generation; the TUI surfaces a write failure as a note while `book-run` fails the run. A record keeps the full trajectory superset (`request` + the reason/tool/observe `llm.CycleEvent`s + the final `Intent` + committed `entry_ids`) **and the rendered `system_prompt` the engine conditioned on**, so the record is self-contained — the mutable tenant context (chart/periods/branches/policy) the teacher saw is captured at run time, not reconstructed later from a `prompt_version` that only versions the template, not its data. From the same corpus a later distillation can train either a tool-calling student or an Intent-only one; the teacher/`prompt_version` provenance keeps records from a changed prompt or model distinguishable (bump `agent.PromptVersion` when the prompt or `Intent` schema changes). It is training telemetry that never touches the authoritative `JournalPosted` log; capture failures degrade to a note on the run, never break the session.
