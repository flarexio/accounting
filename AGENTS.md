...
- **Structural reversal/correction tracking.** A reversal entry is linked to its original through a `JournalRelation` (append-only, composite identity `(from_entry, to_entry)`), not a description string. Relations ride on the same `JournalPosted` event as the reversing entry and `Apply` writes both in one transaction, so the projection never sees an entry without its relation. The same model carries `ClosePeriod`'s M:N period-end relationships under the `closes` discriminator (one closing entry → many original revenue/expense entries); future operations (settlement, adjustments) reuse the same table under their own discriminators rather than each spawning a new one.
...
- **Account search is semantic over enriched text.** `find_accounts` takes a natural-language description of the economic event (`AccountFilter.Query`), not a literal name substring — the tool schema says so, so the model describes the meaning instead of guessing the exact account name. `accounting.AccountEmbeddingText` indexes each account's name, optional `Description`, and `Aliases` (everyday transaction wording), and deliberately excludes the code so its digits do not dilute the semantic vector. `Aliases`/`Description` are seed-time indexing hints: the projection persists only the resulting embedding, not the raw hints, so re-embedding requires re-seeding rather than a schema column. Exact code lookup stays on `LedgerRepository.Account`; a lexical/hybrid channel (matching codes and exact names, fused with the dense ranking) is the planned next step for large charts.
