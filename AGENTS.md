...
- **Reference data is seeded through events too.** `ledger seed` does not write the repository directly: the `SeedScenario` use case publishes one event per entity (`CompanyConfigured`, `AccountAdded`, `BranchAdded`, `PeriodAdded`) and the `ApplyCompany`/`ApplyAccount`/`ApplyBranch`/`ApplyPeriod` handlers upsert them, so the chart lives in the event log and is recoverable. `Scenario.Seed(repo)` remains as the direct projection used in tests. On NATS the projection is async, so `EventBus.CatchUp` blocks until the durable consumer has drained the stream to its head: `buildMessaging` calls it after subscribing (every command starts from a current projection) and `seed` calls it again after publishing (the chart is queryable when `ledger seed` returns). On the in-process bus, which dispatches synchronously on Publish, CatchUp is a no-op.
